...
bool renderPath(ros::NodeHandle node_handle, vector <vector<double> > robotTraj){


  ros::Publisher path_pub = node_handle.advertise<visualization_msgs::Marker>("/map/visualization_marker", 10000, true);
  visualization_msgs::Marker marker;
  marker.header.frame_id = "/map";
  marker.header.stamp = ros::Time();
  marker.ns = "path";
...
int main(int argc, char **argv){
  ...
  if(!renderWorld(n))  // For indoor - use renderWorld
    ...
    visualization_msgs::Marker marker = renderWorldMesh(n) ; 
  ...
  //if(!renderPath(n,robotTraj))
  //	ROS_INFO("Could not render robot trajectory !\n");
