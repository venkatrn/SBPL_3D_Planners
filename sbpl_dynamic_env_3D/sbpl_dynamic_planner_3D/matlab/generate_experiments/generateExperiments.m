...
for i=1:numTests
    ...
    while true
        [startX, startY, startZ] = generateRandomValidPoint(map,0);
        [goalX, goalY, goalZ] = generateRandomValidPoint(map,0);
        %startZ = 1;
        %goalZ = 4;
        %path = a_star(map(:,:,1),[startX startY],[goalX goalY]);
        path = a_star3D(imdilate3D(map,robotRadius),[startX startY startZ],[goalX goalY goalZ]);
        if(numel(path) > 400)  %TODO: check condition
            break;
        end
    end
    ...
    for j=1:numDynObs
        ...
        path = generateDynamicObstacle(map, radius, minPath, startX, startY, startZ, fatFlag);
        ...
        for k=1:size(path,1)
            fprintf(fout, '%f %f %f %f %f\n', (path(k,1)-1)*res, (path(k,2)-1)*res, (path(k,3)-1)*res, (k-1)*timeRes, 0);
        end
